...
1. 工具可以通过 name 调用。
2. 错误参数会被拒绝。
3. 工具调用结果使用标准 ToolResult，并可写入内存 ToolCallTrace；完整 Trace 留到 Phase 16。
...
2. Web 能显示 diff。
3. 用户确认后才写入文件。
4. Patch Proposal 和 patch history 可在 Session Log 中看到；完整 FileEditTrace 留到 Phase 16。
```
...
2. 危险命令不会执行。
3. 审批通过后用户可以应用已批准 Patch。
4. 审批拒绝后 proposal 状态可见；Agent 自动 resume 留到后续 Agent 集成阶段。
```
...
3. 设置 timeout。
4. 记录 exit code。
5. 命令结果进入 command history；进入 Agent observation 留到后续 Agent 集成阶段。
6. Web 展示命令日志。
```
...
```text
1. Web/Server 可以通过 Shell Runner 运行 npm run typecheck。
2. timeout 生效。
3. exit code 非 0 时不会崩溃。
4. stdout/stderr 会被截断保存并在 Web 展示。
```
...
```text
1. 运行安全验证命令。
2. 基于已知 TypeScript 错误形态定位相关文件。
3. 生成 Patch Proposal，等待人工审批。
4. 用户应用已批准 Patch 后再次验证。
5. 保存 command、patch、approval、repair attempt 记录。
```

验收：

```text
1. example 项目初始 typecheck 失败，应用 Patch 后可以变成功。
2. 修复器只生成一个受控 Patch Proposal，不直接写文件。
3. Session Log 包含 command、patch、approval、repair attempt。
4. 当前不实现多轮自动 retry 或失败报告；这些能力留到 Agent 集成与完整 Trace 阶段。
```
...
6. 有 approval。
7. 有 shell runner。
8. 有 session log、step log、command history；完整 Trace 留到 Phase 16。
